...
export async function listProjects(owner: string, repo: string, opts?: { isAuthed?: boolean | null }): Promise<Project[]> {
  const data = await tryAuthedFirst<{ projects?: Project[] }>(`${authedBase(owner, repo)}/projects`, `${publicBase(owner, repo)}/projects`, opts?.isAuthed);
  return data.projects ?? [];
}

export async function loadProjectBoard(owner: string, repo: string, number: number, opts?: { isAuthed?: boolean | null }): Promise<ProjectBoard> {
...
export async function createProject(owner: string, repo: string, input: { title: string; description?: string | null }): Promise<{ project: Project }> {
  return apiPost(`${authedBase(owner, repo)}/projects`, input);
}

export async function updateProject(owner: string, repo: string, number: number, patch: { title?: string; description?: string | null; status?: 'open' | 'closed' }): Promise<{ project: Project }> {
...
export async function renameColumn(owner: string, repo: string, number: number, columnId: string, title: string): Promise<{ column: ProjectColumn }> {
...
export async function createCard(
  ...
  repo: string,
  number: number,
  input: { columnId: string; kind?: 'note' | 'issue' | 'pull'; noteTitle?: string; noteBody?: string; issueId?: string; pullRequestId?: string },
): Promise<{ card: ProjectCard }> {
  return apiPost(`${authedBase(owner, repo)}/projects/${number}/cards`, input);
}

export async function moveCard(owner: string, repo: string, number: number, cardId: string, input: { toColumnId: string; position?: number }): Promise<{ card: ProjectCard }> {
  return apiPatch(`${authedBase(owner, repo)}/projects/${number}/cards/${encodeURIComponent(cardId)}/move`, input);
}

export async function setCardArchived(owner: string, repo: string, number: number, cardId: string, archived: boolean): Promise<{ card: ProjectCard }> {
