...
- Protected (`/user/*`): `GET /user/me` (`{email,username,displayName}`) · `PATCH /user/me` · `PATCH /user/me/username` (cascades repos, best-effort DO move, frees old name) · `GET|POST /user/repos` (owner defaults to self username; org owner must be org where caller is owner|member) · `GET|PATCH|DELETE /user/repos/:owner/:repo` (`admin` for write ops, `viewerRole/viewerCanManage` in reads) (+ `/branches|/tree|/blob|/commits` DO passthroughs) · `POST /user/repos/:owner/:repo/contents` (upsert text file, `201` on create) · `DELETE /user/repos/:owner/:repo/contents?branch=&path=` (file delete; both `write+`, optional `expectedOid` → 409) · `GET|PUT|DELETE /user/repos/:owner/:repo/collaborators[/:member]` (`admin`-only, `admin|write|read`) · `POST|GET|PATCH|DELETE /user/orgs[/:org[/members[/:member]]]` (member mgmt `owner`-only, last-owner guard) · `GET|POST|DELETE /user/tokens` · `GET|POST /user/.../issues`, `GET|PATCH /user/.../issues/:number` (`write+` to triage) · `GET|POST /user/.../issues/:number/comments`.
