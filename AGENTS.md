...
- **Storage**: `apps/background` `RepoWorker` facade (one per repo, `getByName(fullName)`, 5GB, `/repo` bare; lifecycle in `RepoLifecycle`, reads in `RepoReadRpc`) + `CronTasksWorker` (`*/10 * * * *`, token prune); D1 `migrations/0021_squash.sql` (single baseline squashed from 0001–0020: users, repos, PATs, issues, pulls, FTS, webhooks, releases, teams, audit, checks).
...
- **Identity privacy**: email is the stable unique ID (usernames are mutable and renames must not orphan history); D1 keeps `*_email`/`created_by` store keys untouched, API responses expose only current usernames via `IdentityResolver` (`packages/backend-services/src/identity/`, `Tokens.IdentityResolver`, batch `UserDAO.getByEmails/getByUsernames`, miss → `ghost`) + `presentMany/presentSingle` in `apps/api/src/workers/routes/IdentityPresenter.ts` (`creator/author/actor/username`, `mergedBy/resolvedBy/dismissedBy`, `createdBy` as username; audit `userEmail` → `username`, filters accept `username|email`); only `GET /user/me` returns self email.
