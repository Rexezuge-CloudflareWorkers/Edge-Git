...
function json(init?: RequestInit & { body?: unknown }): RequestInit {
  return {
    ...
    body: JSON.stringify((init as { body?: unknown })?.body ?? {}),
  };
}
...
describe('transfer + hardening lifecycle on real D1', () => {
  ...
  it('rejects bad import URLs and tracks jobs', async () => {
    ...
    const started = await api(
      `/user/repos/${OWNER}/${REPO_A}/import`,
      json({ method: 'POST', body: { sourceUrl: 'https://example.invalid/o/r' } }),
    );
    ...
    const second = await api(
      `/user/repos/${OWNER}/${REPO_A}/import`,
      json({ method: 'POST', body: { sourceUrl: 'https://example.invalid/o/other' } }),
    );
  ...
  it('configures, toggles, and removes mirrors', async () => {
    const badInterval = await api(
      `/user/repos/${OWNER}/${REPO_A}/mirror`,
      json({ method: 'PUT', body: { sourceUrl: 'https://example.invalid/o/r', intervalMinutes: 61 } }),
    );
    expect(badInterval.status).toBe(400);

    const put = await api(
      `/user/repos/${OWNER}/${REPO_A}/mirror`,
      json({ method: 'PUT', body: { sourceUrl: 'https://example.invalid/o/r', intervalMinutes: 60 } }),
    );
    ...
    const disabled = (await (
      await api(`/user/repos/${OWNER}/${REPO_A}/mirror/enable`, json({ method: 'POST', body: { enabled: false } }))
    ).json()) as {
  ...
  it('mints deploy keys that unlock git and revoke cleanly', async () => {
    ...
    const listed = (await (await api(`/user/repos/${OWNER}/${REPO_A}/keys`)).json()) as {
      keys: Array<{ id: string; tokenPrefix: string | null }>;
    };
    ...
    expect(
      (await api(`/${OWNER}/${REPO_A}/info/refs?service=git-receive-pack`, { headers: { Authorization: `Bearer ${created.key}` } })).status,
    ).toBe(401);
    // Keys are scoped to their repo.
    expect(
      (await api(`/${OWNER}/${REPO_B}/info/refs?service=git-upload-pack`, { headers: { Authorization: `Bearer ${created.key}` } })).status,
    ).toBe(401);
  ...
  it('serves security settings with warn default and allows block', async () => {
    ...
    const blocked = (await (
      await api(`/user/repos/${OWNER}/${REPO_A}/security`, json({ method: 'PATCH', body: { secretScanMode: 'block' } }))
    ).json()) as {
    ...
    const back = (await (
      await api(`/user/repos/${OWNER}/${REPO_A}/security`, json({ method: 'PATCH', body: { secretScanMode: 'warn' } }))
    ).json()) as {
      settings: { secretScanMode: string };
    };
  ...
  it('rotates PATs and enforces repo grants', async () => {
    const mint = (await (await api('/user/tokens', json({ method: 'POST', body: { name: 'rot-me' } }))).json()) as {
      tokenId: string;
      token: string;
      prefix: string;
    };
    ...
    const rotated = (await (await api(`/user/tokens/${mint.tokenId}/rotate`, { method: 'POST' })).json()) as {
      token: string;
      expiresAt: number;
    };
    ...
    const scoped = (await (
      await api(
        '/user/tokens',
        json({ method: 'POST', body: { name: 'scoped', repoGrants: [{ owner: OWNER, name: REPO_A, scope: 'repo:read' }] } }),
      )
    ).json()) as { token: string };
    expect(
      (await api(`/${OWNER}/${REPO_A}/info/refs?service=git-upload-pack`, { headers: { Authorization: `Bearer ${scoped.token}` } })).status,
    ).toBe(200);
    // Read grant: push advertise is forbidden …
    expect(
      (await api(`/${OWNER}/${REPO_A}/info/refs?service=git-receive-pack`, { headers: { Authorization: `Bearer ${scoped.token}` } }))
        .status,
    ).toBe(403);
    // … and repos outside the grant set hide their existence.
    expect(
      (await api(`/${OWNER}/${REPO_B}/info/refs?service=git-upload-pack`, { headers: { Authorization: `Bearer ${scoped.token}` } })).status,
    ).toBe(401);

    const badGrant = await api(
      '/user/tokens',
      json({ method: 'POST', body: { name: 'bad-grant', repoGrants: [{ owner: 'nobody', name: 'missing', scope: 'repo:read' }] } }),
    );
