...
function createThreadFakes(overrides: { prStatus?: string } = {}) {
  const threads: Array<Record<string, unknown>> = [
    { id: 't-1', pull_request_id: 'pr-1', path: 'src/app.ts', line: 10, side: 'new', commit_oid: null, status: 'open', author_email: 'alice@example.com', created_at: 1, resolved_by: null, resolved_at: null },
  ];
  const comments: Array<Record<string, unknown>> = [{ id: 'c-1', thread_id: 't-1', author_email: 'alice@example.com', body: 'first', created_at: 1 }];
  ...
  const pullThreadDAO = {
    openThread: (input: Record<string, unknown>) => {
      threads.push({ id: input.id, pull_request_id: input.pullRequestId, path: input.path, line: input.line, side: input.side, commit_oid: input.commitOid, status: 'open', author_email: input.authorEmail, created_at: input.now, resolved_by: null, resolved_at: null });
      return Promise.resolve();
    },
    listThreads: (pullRequestId: string) => Promise.resolve(threads.filter((t) => t.pull_request_id === pullRequestId)),
    getThread: (pullRequestId: string, threadId: string) => Promise.resolve(threads.find((t) => t.pull_request_id === pullRequestId && t.id === threadId) ?? null),
    ...
    resolveThread: (threadId: string, by: string, resolved: boolean, now: number) => {
      ...
      if (row) Object.assign(row, { status: resolved ? 'resolved' : 'open', resolved_by: resolved ? by : null, resolved_at: resolved ? now : null });
      return Promise.resolve();
    },
  };
...
function threadService(fakes: ReturnType<typeof createThreadFakes>) {
  return new PullThreadService({ DB: {} as D1Queryable }, {
...
describe('PullThreadService threads', () => {
  it('opens a thread with an initial comment', async () => {
    ...
    const thread = await svc.openThread({ pullRequestId: 'pr-1', authorEmail: 'bob@example.com', path: 'src/app.ts', line: 42, side: 'new', body: 'nit?' });
  ...
  it('validates thread input', async () => {
    ...
    await expect(svc.openThread({ pullRequestId: 'pr-1', authorEmail: 'a@b.c', path: 'a.ts', side: 'middle', body: 'x' })).rejects.toThrow();
    await expect(svc.openThread({ pullRequestId: 'pr-1', authorEmail: 'a@b.c', path: 'a.ts', body: '   ' })).rejects.toThrow();
    await expect(svc.openThread({ pullRequestId: 'missing', authorEmail: 'a@b.c', path: 'a.ts', body: 'x' })).rejects.toThrow('Pull request not found');
  });
  ...
  it('lists threads with embedded comments and resolves them', async () => {
    ...
    await expect(svc.replyThread({ pullRequestId: 'pr-1', threadId: 'missing', authorEmail: 'b@c.d', body: 'x' })).rejects.toThrow('Thread not found');
    ...
    await expect(svc.resolveThread({ pullRequestId: 'pr-1', threadId: 'missing', resolvedBy: 'a@b.c', resolved: true })).rejects.toThrow('Thread not found');
  });
});

function createDismissFakeDao() {
  const reviews: Array<Record<string, unknown>> = [
    { id: 'r-1', pull_request_id: 'pr-1', author_email: 'bob@example.com', state: 'changes_requested', body: null, commit_oid: null, created_at: 1, dismissed: 0 },
  ];
  return {
    reviews,
    getByNumber: () => Promise.resolve({ ...PR }),
    listReviews: () => Promise.resolve(reviews),
    getReviewById: (pullRequestId: string, reviewId: string) => Promise.resolve(reviews.find((r) => r.pull_request_id === pullRequestId && r.id === reviewId) ?? null),
...
describe('PullRequestService dismissReview', () => {
  it('dismisses a blocking review so the gate clears', async () => {
    ...
    const dismissed = await svc.dismissReview({ repositoryId: 'repo-1', number: 1, reviewId: 'r-1', dismissedBy: 'alice@example.com', reason: 'outdated' });
    expect(dismissed.dismissed).toBe(1);
    expect(PullRequestService.isBlockedByReviews(await fake.listReviews())).toBe(false);
    await expect(svc.dismissReview({ repositoryId: 'repo-1', number: 1, reviewId: 'r-1', dismissedBy: 'alice@example.com' })).rejects.toThrow('already dismissed');
    await expect(svc.dismissReview({ repositoryId: 'repo-1', number: 1, reviewId: 'missing', dismissedBy: 'alice@example.com' })).rejects.toThrow('Review not found');
  });
...
describe('PullThreadDAO SQL', () => {
  function createDaoFakeDb() {
    const state = { threads: [] as Array<Record<string, unknown>>, comments: [] as Array<Record<string, unknown>>, reviews: [] as Array<Record<string, unknown>> };
    state.reviews.push({ id: 'r-1', pull_request_id: 'pr-1', author_email: 'b@c.d', state: 'approved', body: null, commit_oid: null, created_at: 1, dismissed: 0 });
      ...
      return {
        ...
        run(): Promise<{ success: boolean }> {
          if (q.startsWith('INSERT INTO pull_review_threads')) {
            const [id, pull_request_id, path, line, side, commit_oid, status, author_email, created_at] = params as Array<string | number | null>;
            state.threads.push({ id, pull_request_id, path, line, side, commit_oid, status, author_email, created_at, resolved_by: null, resolved_at: null });
    ...
    return { db: { prepare: (query: string) => ({ bind: (...params: unknown[]) => statement(query, params) }) } as unknown as D1Queryable, state };
  }

  it('round-trips threads through SQL', async () => {
    const { db, state } = createDaoFakeDb();
    const dao = new PullThreadDAO(db);
    await dao.openThread({ id: 't-9', pullRequestId: 'pr-1', path: 'a.ts', line: 3, side: 'old', commitOid: null, authorEmail: 'a@b.c', now: 7 });
  ...
  it('degrades gracefully on legacy DBs without thread tables', async () => {    const db = {
      prepare: () => ({
        bind: () => ({
  ...
  it('dismisses reviews through SQL', async () => {
    const reviews: Array<Record<string, unknown>> = [
      { id: 'r-1', pull_request_id: 'pr-1', author_email: 'b@c.d', state: 'approved', body: null, commit_oid: null, created_at: 1, dismissed: 0 },
    ];
    const db = {
      prepare: (query: string) => ({
        bind: (...params: unknown[]) => {
          ...
          if (q.startsWith('SELECT')) {
            return { first: () => Promise.resolve((reviews.find((r) => r.pull_request_id === params[0] && r.id === params[1]) ?? null) as unknown) };
          }
          const [by, at, reason, id] = params as Array<string | number | null>;
